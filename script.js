...
if (searchForm && searchInput && searchResult) {
  searchForm.addEventListener("submit", (event) => {
    ...
    if (!value) {
      searchResult.textContent = "Ievadi dzīvnieka nosaukumu meklēšanai.";
      searchResult.className = "search-result is-error";
      return;
    }

    const animal = animals[normalizeAnimalName(value)];

    if (animal) {
      renderAnimal(animal);
      searchResult.textContent = `${animal.name} atrasts! Informācija par šo dzīvnieku redzama jaunajā sadaļā zem meklētāja.`;
      searchResult.className = "search-result is-success";
    } else {
      searchResult.textContent = `Dzīvnieks "${value}" netika atrasts. Pamēģini: Lauva, Zilonis, Žirafe, Nilzirgs vai Zebra.`;
      searchResult.className = "search-result is-error";
    }
  });
}
